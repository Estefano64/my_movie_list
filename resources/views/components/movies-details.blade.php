<div class="bg-white block mt-6 p-6 border-gray-300 hover:border-indigo-300 hover:ring hover:ring-indigo-200 hover:ring-opacity-50 rounded-md shadow-sm">
    <div class="flex justify-end">
        <x-dropdown>
            ...
            <x-slot name="content">
                <x-dropdown-link :href="route('movies.show', $movie)">
                    Ver
                </x-dropdown-link>
                <form method="POST" action="{{ route('movies.destroy', $movie) }}">
                    @csrf
                    @method('DELETE')
                    <x-dropdown-link :href="route('movies.destroy', $movie)" onclick="event.preventDefault(); this.closest('form').submit();">
                        Eliminar
                    </x-dropdown-link>
                </form>
            </x-slot>
    ...
    <div class="flex items-center">
        <div class="flex-shrink-0">
            @if ($movie->cover_image)
                <img src="{{ asset('storage/' . $movie->cover_image) }}" alt="Movie Cover" class="h-12 w-12 rounded-full object-cover">
            @else
        ...
        <div class="flex-1 min-w-0 ms-4">
            <p class="text-sm font-medium text-gray-900 truncate dark:text-black">{{ $movie->name }}</p>
            <p class="text-sm text-gray-500 truncate dark:text-gray-400">{{ $movie->release_year }}</p>
        </div>
    </div>
</div>
